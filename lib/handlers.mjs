...
handlers._users.get = (data, callbackFn) => {
  ...
  if (phone) {
    ...
    // Verify given token is valid for the given phone
    handlers._token.verifyToken(token, phone, (isValidToken) => {
      if (isValidToken) {
    ...
    handlers._token.verifyToken(token, phone, (isValidToken) => {
      if (isValidToken) {
        if (firstName || lastName || password) {
          ...
          callbackFn(400, {
            error: "Missing required field or fields to update",
          });
        }
      } else {
        callbackFn(403, {
      ...
      if ((!err, tokenData)) {
        ...
        _data.delete("tokens", id, (err) => {
          if (!err) {
            ...
            callbackFn(500, {
              error: "Error occurred while deleting the token",
            });
          }
        });
      } else {
...
handlers._token.verifyToken = (id, phone, callbackFn) => {
  _data.read("tokens", id, (err, tokenData) => {
    if (!err && tokenData) {
      if (
        tokenData.phone.toString() === phone.toString() &&
